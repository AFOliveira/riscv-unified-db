...
# Source: RISC-V Unprivileged ISA Manual
#   https://github.com/riscv/riscv-isa-manual/blob/main/src/rv-32-64g.adoc
#   Encoding format tables (R/I/S/B/U/J-type with bit column headers)
#   R4-type: rs3 at bits [31:27] (used in fused multiply-add instructions)
#
# Standard positions:
...
# Maps standard RISC-V assembly register operands to their encoding field names.
ASM_TO_VAR = {
  "xd"  => %w[rd xd],
  "xs1" => %w[rs1 xs1],
  "xs2" => %w[rs2 xs2],
...
#
# Classifies each custom instruction's opcode (bits[6:0]) against the standard
# RISC-V opcode map to determine if it uses custom, standard, or reserved space.
#
# Source: RISC-V Unprivileged ISA Manual
#   https://github.com/riscv/riscv-isa-manual/blob/main/src/rv-32-64g.adoc
#   Section: [[opcodemap]] "RISC-V base opcode map, inst[1:0]=11"
...
#
# Detects encoding overlaps between custom and standard instructions.
# Advisory check — not a specific ISA rule, but custom instructions that are
# indistinguishable from standard ones would cause decoder ambiguity.
...
# Check 3: Variable Name Convention
#
# Verifies that standard register variable names (rs1, rs2, rd, rs3, etc.)
# appear at their ISA-mandated bit positions in the encoding.
#
# Source: RISC-V Unprivileged ISA Manual
#   https://github.com/riscv/riscv-isa-manual/blob/main/src/rv-32-64g.adoc
#   Encoding format tables: R/I/S/B/U/J-type (rd@11:7, rs1@19:15, rs2@24:20)
#   R4-type (rs3@31:27, used in fused multiply-add)
...
#
# Verifies that assembly operands have corresponding encoding variables.
# Advisory check — internal consistency between the assembly syntax string
# and the encoding field definitions.
...
#
# Validates structural encoding properties:
# - bits[1:0] must be '11' for 32-bit encodings (RISC-V ISA requirement)
# - match string must be a valid instruction width (16, 32, or 48 bits)
# - every '-' (don't-care) bit must be covered by a decode variable
# - every fixed bit ('0'/'1') must not be covered by a decode variable
#
# Source: RISC-V Unprivileged ISA Manual
#   https://github.com/riscv/riscv-isa-manual/blob/main/src/intro.adoc
...
def check_encoding_format_integrity(inst, xlen, results)
  ...
  # Check bits[1:0] == 11 for 32-bit encodings (RISC-V ISA requirement)
  if format_str.size == 32
    low_bits = format_str[-2..]
    if low_bits != "11"
      results.add("5. Encoding Format Integrity", "warn", inst.name,
        "bits[1:0]='#{low_bits}' but expected '11' for 32-bit encoding")
    end
  end
...
#
# Verifies that the instruction file lives under a directory matching one of
# its definedBy extension names. This is a UDB convention, not an ISA rule.
