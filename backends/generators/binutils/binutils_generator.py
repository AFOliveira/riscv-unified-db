...
def generate_binutils_opcodes(
    ...
    for name, info in sorted(instr_dict.items(), key=lambda x: x[0].upper()):
        ...
        try:
            ...
            if insn_class.startswith("INSN_CLASS_") and not is_user_defined_class(
                insn_class
            ):
...
def generate_extension_logic(extension_def):
    if isinstance(extension_def, str):
        ...
        if "anyOf" in extension_def:
            ...
            checks = [
                f'riscv_subset_supports (rps, "{ext.lower()}")' for ext in extensions
            ]
...
def parse_args():
    ...
    parser.add_argument(
        "--extensions",
        default="I,M,A,F,D,C,Zba,Zbb,Zbs,Zca",
